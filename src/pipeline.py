...
def run(config: Config | None = None, seed: int = 0) -> PolicyValueNet:
    ...
    终止条件：累计自对弈局数达到 config.total_games。返回训练后的网络。
    """
    ...
    print(f"device={device} workers={n_workers} total_games={config.total_games}")
    games = 0
    steps = 0
    try:
        while games < config.total_games:
            # 把当前可取的整局样本全部吃进 buffer（无则阻塞等一局）。
            drained = 0
            while True:
                try:
                    buffer.add(sample_q.get_nowait())
                    games += 1
                    drained += 1
                except Empty:
                    break
            if drained == 0:
                try:
                    buffer.add(sample_q.get(timeout=2.0))
                    games += 1
                except Empty:
                    continue
            ...
            print(
                f"games={games} steps={steps} buffer={len(buffer)} "
