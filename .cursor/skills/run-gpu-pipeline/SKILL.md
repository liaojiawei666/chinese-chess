...
description: >-
  ...
  （datagen，tch-rs GPU 推理）、arena 比赛评价三者并跑。当用户说要「在 GPU 上跑」「跑训练
...
`config/gpu.json` 里的 `data/samples`、`data/models` 是相对路径，cwd 不一致就会各写各的目录、对不上。
...
2. **确认源码里有 torch_cuda.dll 预加载补丁**（Windows GPU 自对弈的关键，见下文「核心坑 3」）。
   检查 `crates/core/src/infer/torch_model.rs` 的 `TorchModel::load` 是否在 CUDA 设备时调用
...
```powershell
# 1) 训练（GPU）
& "$venv\Scripts\python.exe" trainer\scripts\train_loop.py --config config\gpu.json

# 2) 自对弈数据生成（GPU 推理）
cargo --manifest-path crates\Cargo.toml run --release -p datagen -- --config config\gpu.json

# 3) arena 比赛评价（默认 CPU，不抢卡；等 ≥2 个 checkpoint 后自动对杀）
& "$venv\Scripts\python.exe" trainer\scripts\arena_daemon.py --config config\gpu.json --checkpoints-only
```
...
```powershell
Remove-Item -Recurse -Force crates\target\release\build\torch-sys-*,crates\target\release\build\tch-*
Remove-Item -Force crates\target\release\deps\*torch_sys*,crates\target\release\deps\*libtch*
...
`crates/core/src/infer/torch_model.rs`：
...
```powershell
Remove-Item -Force crates\target\release\deps\*inference*,crates\target\release\datagen.* ,crates\target\release\deps\datagen*
cargo --manifest-path crates\Cargo.toml build --release -p datagen
```
