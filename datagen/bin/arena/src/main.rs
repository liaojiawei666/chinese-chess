...
//!   cargo run -p arena --features torch -- --num-openings 100 --report data/arena/report.json
//!   # 按版本号
//!   cargo run -p arena --features torch -- --version-a 100 --version-b 40
//!   # 显式路径
//!   cargo run -p arena --features torch -- \
...
impl Default for Args {
    fn default() -> Self {
        Args {
            run_config: PathBuf::from("data/config/run-config.local.json"),
...
fn parse_args() -> Result<Args> {
    ...
    while i < argv.len() {
        match argv[i].as_str() {
            "--run-config" => args.run_config = PathBuf::from(need(&argv, i)?),
...
fn print_help() {
    println!(
        "arena —— 两模型对杀，输出 A 相对 B 的 Elo（纯指标，不改 latest.json）\n\n\
         ...
         \x20 --run-config <path>     默认 data/config/run-config.local.json（取 mcts/rules/device）\n\
...
fn main() -> Result<()> {
    ...
    let text = std::fs::read_to_string(&args.run_config)
        .with_context(|| format!("读取 run-config 失败：{}", args.run_config.display()))?;
    ...
    let report = run_players(&mut args, &device, &openings, mcts_config, rc.rules.max_total_plies)?;
...
fn resolve_models(args: &Args) -> Result<(String, String)> {
    ...
    println!("自动选版：A = v{va}（最新） vs B = v{vb}（上一版）");
    Ok((
        store.path_for(va).to_string_lossy().into_owned(),
        store.path_for(vb).to_string_lossy().into_owned(),
    ))
...
fn run_players(
    ...
    eprintln!(
        "⚠ 未启用 torch 特性：以均匀评估器跑 A=B 自检（应对称、得分率≈0.5）。\n\
         \x20 真实对杀请加 --features torch 并传 --model-a/--model-b。"
    );
    let eval_a = eval::UniformEvaluator;
    let eval_b = eval::UniformEvaluator;
    Ok(run_match(openings, &eval_a, &eval_b, mcts_config, max_total_plies))
}
