...
use engine::{Color, GameState, Move, Position};
...
fn play_one<E: Evaluator + Copy>(
    ...
    let mut mcts_red = Mcts::new(red, mcts_config, StdRng::seed_from_u64(1));
    let mut mcts_black = Mcts::new(black, mcts_config, StdRng::seed_from_u64(2));

    let mut ply = state.history.len() as u32;
    while ply < max_total_plies {
        let counts = match state.position.side_to_move {
            Color::Red => mcts_red.run(state.clone()),
            Color::Black => mcts_black.run(state.clone()),
        };
        if counts.is_empty() {
            break; // 根节点终局
        }
