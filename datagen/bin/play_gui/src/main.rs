...
impl Default for Args {
    fn default() -> Self {
        Args {
            run_config: PathBuf::from("data/config/run-config.gpu.json"),
...
fn print_help() {
    println!(
        "play_gui —— 人机对战 GUI，默认读取 data/models/latest.json\n\n\
         可选：\n\
         \x20 --run-config <path>  默认 data/config/run-config.gpu.json\n\
...
fn search_ai_move_inner(state: GameState, config: PlayConfig) -> Result<AiResult> {
    use store::{LocalModelStore, ModelStore};
...
fn search_ai_move(state: GameState, config: PlayConfig) -> AiResult {
    ...
    match counts.into_iter().max_by_key(|(_, n)| *n) {
        Some((mv, visits)) => AiResult {
            ...
            model_version: None,
            visits,
            error: Some("未启用 torch feature：当前使用均匀评估器自检。".to_string()),
        },
        None => AiResult {
            mv: None,
            model_version: None,
