...
fn visit_counts_sum_to_simulations() {
    let mut mcts = Mcts::new(config(64), StdRng::seed_from_u64(0));
    let counts = mcts.run(GameState::from_position(Position::starting()), &Uniform);
...
fn returns_one_entry_per_legal_move_and_all_legal() {
    ...
    let mut mcts = Mcts::new(config(32), StdRng::seed_from_u64(1));
    let counts = mcts.run(state, &Uniform);
...
fn leaf_parallel_visit_counts_sum_to_simulations() {
    ...
    for width in [2u32, 8, 7] {
        ...
        let mut mcts = Mcts::new(cfg, StdRng::seed_from_u64(width as u64));
        let counts = mcts.run(GameState::from_position(Position::starting()), &Uniform);
        let total: u32 = counts.iter().map(|(_, n)| *n).sum();
...
fn terminal_root_returns_empty() {
    ...
    let mut mcts = Mcts::new(config(16), StdRng::seed_from_u64(2));
    assert!(mcts.run(state, &Uniform).is_empty());
}
